...
const rules = {
  name: [
    ...
    { min: 0, max: 200, message: '不能超过200个字符', trigger: ['blur', 'change'] }
  ],
  code: [
    { required: true, message: '必填项', trigger: ['blur', 'change'] },
    { min: 0, max: 200, message: '不能超过200个字符', trigger: ['blur', 'change'] }
  ...
  price: [
    { required: true, message: '必填项', trigger: ['blur', 'change'] },
    { pattern: /^([1-9]\d{0,5}|0)(\.\d{1,2})?$/, message: '请输入正确数字，不能超过两位小数、不能大于10万元' }
  ],
  pricostPricece: [
    { required: true, message: '必填项', trigger: ['blur', 'change'] },
    { pattern: /^([1-9]\d{0,5}|0)(\.\d{1,2})?$/, message: '请输入正确数字，不能超过两位小数、不能大于10万元' }
...
import { commodityCreate, commodityModify } from '@/api'
