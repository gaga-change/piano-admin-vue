...
<script>
import { productsList, productsModify } from "@/api";
...
export default {
  name: 'productList',
  components: { productFormDialog, TableTimeDialog },
  data() {
    return {
  ...
  methods: {
    ...
    /** 删除 */
    handleDelete(row) {
      this.$apiConfirm(`是否确定${row.disabled ? '启用' : '禁用'}【${row.name}】？`, () => productsModify(row._id, { disabled: !row.disabled })).then(res => {
        if (!res) return
