...
<script>
import { classTimesList, classTimesModify } from "@/api";
import classTimeFormDialog from "./classTimeFormDialog";
import TableTimeDialog from "@/components/TableTimeDialog"

const tableConfig = [
  { label: "名称", prop: "name", width: 120 },
  { label: "时长（分钟）", prop: "time" },
...
export default {
  name: 'classTimeList',
  components: { classTimeFormDialog, TableTimeDialog },
  data() {
    return {
  ...
  methods: {
    ...
    /** 删除 */
    handleDelete(row) {
      this.$apiConfirm(`是否确定${row.disabled ? '启用' : '禁用'}【${row.name}】？`, () => classTimesModify(row._id, { disabled: !row.disabled })).then(res => {
        if (!res) return
